...
import { GYROUSDT_LPBOND_NAME, PCS_GYROUSDT_PAIR } from "./utils/Constants";
...
export function handleBondDeposit(call: DepositCall): void {
  ...
  let token = loadOrCreateToken(GYROUSDT_LPBOND_NAME);
  ...
  deposit.value = getPairUSD(call.inputs.amount_, PCS_GYROUSDT_PAIR);
...
export function handleBondRedeem(call: RedeemCall): void {
  ...
  redemption.token = loadOrCreateToken(GYROUSDT_LPBOND_NAME).id;
